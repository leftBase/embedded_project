...
int main(void) {
    ...
    // 시리얼 장치 열기 이거 3키짜리 
    button3_fd = open("/dev/ttymxc1", O_RDWR);
    if (button3_fd == -1) return panic();
    ...
    // 시리얼 통신 설정
    if (tcsetattr(button3_fd, TCSANOW, &tio) != 0) return panic();
    ...
    while (1) {
        // 데이터 수신
        int num_bytes = read(button3_fd, &read_buf, sizeof(read_buf));
